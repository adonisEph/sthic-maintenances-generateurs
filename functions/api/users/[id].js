...
export async function onRequestPatch({ request, env, data, params }) {
  ...
  if (!['admin', 'viewer', 'controller', 'field_supervisor', 'technician', 'manager', 'manager_bzv_pool', 'warehouse'].includes(nextRole)) return json({ error: 'Rôle invalide.' }, { status: 400 });
