...
export async function onRequestGet({ request, env, data }) {
  try {
    ...
    return json({ movements, stock, scopeZone: canAllZones ? null : requesterZone }, { status: 200 });
  } catch (e) {
...
export async function onRequestPost({ request, env, data }) {
  try {
    ...
    if (movementType === 'exit') {
      const stockRow = await env.DB.prepare(
        `SELECT COALESCE(SUM(CASE WHEN movement_type = 'entry' THEN quantity ELSE -quantity END), 0) AS current_stock
         FROM stock_movements WHERE item_type = ? AND zone = ?`
      ).bind(itemType, zone).first();
    ...
    await env.DB.prepare(
      `INSERT INTO stock_movements (id, item_type, movement_type, quantity, zone, site_id, site_name, fiche_id, technician_name, notes, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
      .bind(id, itemType, movementType, quantity, zone, siteId, siteName, ficheId, technicianName, notes, data?.user?.email ? String(data.user.email) : null, now)
      .run();
